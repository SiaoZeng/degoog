...
router.get("/api/search", async (c) => {
  ...
  cache.set(key, response, cache.hasFailedEngines(response) ? cache.SHORT_TTL_MS : undefined);
...
router.get("/api/search/retry", async (c) => {
  ...
  if (!query || !engineName) return c.json({ error: "Missing 'q' or 'engine' parameter" }, 400);
  ...
  if (cached) {
    ...
    const merged = newResults.length > 0 ? mergeNewResults(cached.results, newResults) : cached.results;
    const updated = {
      ...cached,
      results: merged,
    ...
    cache.set(key, updated, cache.hasFailedEngines(updated) ? cache.SHORT_TTL_MS : undefined);

...
import { getDefaultEngineConfig, listEngineIds } from "../extensions/engines/registry";
...
export function parseEngineConfig(query: URLSearchParams): EngineConfig {
  const config: EngineConfig = { ...getDefaultEngineConfig() };
  for (const id of listEngineIds()) {
    const value = query.get(id);
    if (value != null) config[id] = value !== "false";
    else if (!(id in config)) config[id] = true;
  }
  return config;
}
